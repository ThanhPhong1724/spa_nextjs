...
export default function HeadspaClient({ content }: { content?: any }) {
    const [openFaq, setOpenFaq] = useState<number | null>(null);
    const { t } = useLanguage();

    const heroTitle = content?.hero?.title_de || "HEADSPA";
    const videoUrl = content?.hero?.video_url || "https://cdn.pixabay.com/video/2020/05/25/40130-424930032_large.mp4";
    const packageImages = content?.package_images ? [
        content.package_images.package1_image,
        content.package_images.package2_image,
        content.package_images.package3_image,
        content.package_images.package4_image,
    ].filter(Boolean) : [];
