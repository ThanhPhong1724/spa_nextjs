            ...
            <section className="py-16 bg-white">
                <div className="max-w-6xl mx-auto px-6">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {[
                            { icon: "💪", title: "Maximale Praxis", desc: "Unbegrenztes Üben an Modellen, bis jeder Handgriff sitzt." },
                            { icon: "💰", title: "Schluss mit Fehlkäufen", desc: "Wir zeigen dir genau, was du wirklich brauchst. Vermeide 100% typische Anfängerfehler." },
                            { icon: "🔗", title: "Best-Preis-Netzwerk", desc: "Zugriff auf unsere Netzwerke für Profi-Zubehör – garantiert beste Preise und höchste Qualität." },
                            { icon: "📊", title: "Business & Media Skills", desc: "Von Steuer-Basics und Studio-Verwaltung bis zur perfekten Social-Media-Präsentation." },
                            { icon: "🚀", title: "Karriere-Boost", desc: "Wir unterstützen dich bei der Jobsuche oder deiner Gründung." },
                            { icon: "✅", title: "Unser Versprechen", desc: "Wir haben die Fehler der letzten 15 Jahre gemacht, damit DU sie nicht machen musst." }
                        ].map((item, i) => (
                        ...
                        {courses.map(course => (
                            <div key={course.id} className="bg-white rounded-2xl shadow-lg overflow-hidden group hover:shadow-xl transition-shadow">
                                ...
                                <div className="p-6">
                                    ...
                                    <p className="text-[#d4a373] text-sm font-medium mb-4">⏱ {course.duration}</p>

                                    <div className="flex flex-wrap gap-2 mb-4">
                                        {course.features.map((f, i) => (
                                            <span key={i} className="bg-[#f8f7f6] text-[#5c4033] text-xs px-2 py-1 rounded">
                                                ✓ {f}
                                            </span>
                                        ))}
                                    </div>
